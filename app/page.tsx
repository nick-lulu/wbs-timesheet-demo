...
const STORAGE_KEY = "corporate-solution-timesheet-v5";
const DEMO_CCB_EMAIL = "jackyzhong@lululemon.com";
const DEMO_CCB_PASSWORD = "123";

const seed: AppState = {
  projects: [
    { code: "CSCOP-417-2", name: "Service Center", track: "Delivery · CSCOP-2", domain: "Fulfillment", owner: "Alice Lang", pm: "Liz Li", vendors: "VT, Baison", initiative: "Service Center Transformation", goal: "Fulfillment delivery excellence", startDate: "2026-01-15", dueDate: "2026-09-30", stage: "GW3 · Development", status: "In progress", pgmComplete: true, locked: false },
    { code: "CSCOP-418-1", name: "Service Center Discovery", track: "Discovery · CSCOP-1", domain: "Fulfillment", owner: "Alice Lang", pm: "Liz Li", vendors: "VT", initiative: "Service Center Transformation", goal: "Fulfillment delivery excellence", startDate: "2026-01-05", dueDate: "2026-04-30", stage: "GW2 · Feasibility", status: "In progress", pgmComplete: true, locked: false },
    { code: "CSCOP-108-1", name: "Weather Alert", track: "Discovery · CSCOP-1", domain: "Corporate", owner: "Bingbing Zhao", pm: "Iris Jin", vendors: "VT, Baison", initiative: "Weather Resilience", goal: "Improve proactive operational awareness", startDate: "2026-02-01", dueDate: "2026-05-31", stage: "GW2 · Feasibility", status: "BRD Signed & Locked", pgmComplete: true, locked: true },
    { code: "CSCOP-965-2", name: "OTB Report CR", track: "Delivery · CSCOP-2", domain: "MP&A", owner: "Hong Min", pm: "Ada Yu", vendors: "Inspire, EY", initiative: "OTB Planning Modernization", goal: "Increase planning accuracy", startDate: "2026-03-01", dueDate: "2026-10-15", stage: "GW4 · Deploy", status: "In progress", pgmComplete: true, locked: false },
    { code: "CSCOP-1027", name: "Keystone R1 – Regression Testing Support", track: "PgM path required", domain: "Upstream SCM&MDM", owner: "Aki Zhu", pm: "Aki", vendors: "MAI, Hand, Softtek", initiative: "", goal: "", startDate: "", dueDate: "", stage: "GW1 · Intake", status: "PgM setup required", pgmComplete: false, locked: false },
  ],
  allocations: [
...
function AllocationWorkspace({ state, role, save }: { state: AppState; role: Role; save: (state: AppState, message: string) => void }) {
  ...
  const readyProjects = assignedProjects.filter(project => project.pgmComplete && !project.locked);
  ...
  const planningEnabled = project.pgmComplete && !project.locked;
  ...
  return <div className="page"><button className="back-link" onClick={() => setCreating(false)}>← Allocation Dashboard</button><section className="selection-strip allocation-selection"><label>Project (CSCOP)<select value={project.code} onChange={event => setProjectCode(event.target.value)}>{assignedProjects.map(item => <option key={item.code} value={item.code}>{item.code} · {item.name}{item.pgmComplete ? "" : " · PgM setup required"}</option>)}</select></label><label>Vendor<select value={role === "Vendor PM" ? CURRENT_VENDOR : vendor} disabled={role === "Vendor PM"} onChange={event => setVendor(event.target.value)}>{vendorOptions.map(item => <option key={item}>{item}</option>)}</select></label><div><span>Plan total</span><strong>{md(total)} MD</strong></div><button className="primary" disabled={!planningEnabled} onClick={submit}>{planningEnabled ? "Submit allocation" : "Allocation locked"}</button></section>{!planningEnabled && <section className="allocation-lock-notice"><strong>Why is this allocation locked?</strong><p>PgM must create the Discovery or Delivery path and complete Initiative, Project, Goal, Start Date, Due Date, Assigned PM and Gateway dates before VT can edit or submit planned Mandays.</p></section>}<section className="panel matrix-panel"><div className="panel-head"><div><h3>Create Manday Allocation</h3><p>{planningEnabled ? `Enter planned Mandays to two decimal places for ${project.code}.` : `${project.code} is assigned to VT. Editing opens automatically after PgM completes Section 1.`}</p></div><Pill tone={planningEnabled ? "green" : "amber"}>{planningEnabled ? "Ready to edit" : "PgM setup required"}</Pill></div><div className="matrix"><div className="matrix-row matrix-head"><span>Epic / Deliverable</span><span>PM</span><span>BA</span><span>TL</span><span>DEV</span><span>QA</span><span>Total</span></div>{epics.map(([stage, rows]) => <div className="stage-group" key={stage}><div className="stage-title">{stage}</div>{rows.map(row => { const id = row.split(" · ")[0]; const rowTotal = [0, 1, 2, 3, 4].reduce((sum, index) => sum + (values[`${id}-${index}`] || 0), 0); return <div className="matrix-row" key={row}><span><b>{row}</b></span>{[0, 1, 2, 3, 4].map(index => <span key={index}><input aria-label={`${row} role ${index}`} className={`matrix-input c${index}`} type="number" min="0" step="0.01" disabled={!planningEnabled} value={values[`${id}-${index}`] ?? ""} onChange={event => setValues({ ...values, [`${id}-${index}`]: Number(event.target.value) })} /></span>)}<span className="total">{rowTotal ? md(rowTotal) : "—"}</span></div>; })}</div>)}</div><div className="matrix-total"><span>Plan total</span><b /><b /><b /><b /><b /><strong>{md(total)} MD</strong></div></section></div>;
}
