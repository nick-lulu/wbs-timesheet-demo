...
function AllocationWorkspace({ state, role, save }: { state: AppState; role: Role; save: (state: AppState, message: string) => void }) {
  ...
  return <div className="page"><button className="back-link" onClick={() => setCreating(false)}>← Allocation Dashboard</button><section className="selection-strip"><label>Project (CSCOP)<select value={project.code} onChange={event => setProjectCode(event.target.value)}>{assignedProjects.map(item => <option key={item.code} value={item.code}>{item.code} · {item.name}{item.pgmComplete ? "" : " · PgM setup required"}</option>)}</select></label><label>Vendor<select value={role === "Vendor PM" ? CURRENT_VENDOR : vendor} disabled={role === "Vendor PM"} onChange={event => setVendor(event.target.value)}>{vendorOptions.map(item => <option key={item}>{item}</option>)}</select></label><label>Quarter<select><option>FY26 Q1</option></select></label><div><span>Plan total</span><strong>{total} MD</strong></div><button className="primary" disabled={!planningEnabled} onClick={submit}>{planningEnabled ? "Submit allocation" : "Allocation locked"}</button></section>{!planningEnabled && <section className="allocation-lock-notice"><strong>Why is this allocation locked?</strong><p>PgM must complete Section 1 — Goal, Initiative, GW1 BRD Sign-off Date and GW4 Tech Release Date — before VT can edit or submit planned Mandays. The project remains visible so VT can confirm the assignment.</p></section>}<section className="panel matrix-panel"><div className="panel-head"><div><h3>Create Manday Allocation</h3><p>{planningEnabled ? `Enter planned Mandays by Epic and role for ${project.code}.` : `${project.code} is assigned to VT. Editing opens automatically after PgM completes Section 1.`}</p></div><Pill tone={planningEnabled ? "green" : "amber"}>{planningEnabled ? "Ready to edit" : "PgM setup required"}</Pill></div><div className="matrix"><div className="matrix-row matrix-head"><span>Epic / Deliverable</span><span>PM</span><span>BA</span><span>TL</span><span>DEV</span><span>QA</span><span>Total</span></div>{epics.map(([stage, rows]) => <div className="stage-group" key={stage}><div className="stage-title">{stage}</div>{rows.map(row => { const id = row.split(" · ")[0]; const rowTotal = [0, 1, 2, 3, 4].reduce((sum, index) => sum + (values[`${id}-${index}`] || 0), 0); return <div className="matrix-row" key={row}><span><b>{row}</b></span>{[0, 1, 2, 3, 4].map(index => <span key={index}><input aria-label={`${row} role ${index}`} className={`matrix-input c${index}`} type="number" min="0" disabled={!planningEnabled} value={values[`${id}-${index}`] || ""} onChange={event => setValues({ ...values, [`${id}-${index}`]: Number(event.target.value) })} /></span>)}<span className="total">{rowTotal || "—"}</span></div>; })}</div>)}</div><div className="matrix-total"><span>Plan total</span><b /><b /><b /><b /><b /><strong>{total} MD</strong></div></section></div>;
}
